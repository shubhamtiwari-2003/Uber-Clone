import React from 'react'

const WaitingForDriver = (props) => {
  return (
    <div>
      <h5
        onClick={() => {
          props.setWaitingforDriverPanel(false);
        }}
        className=" hover:text-red-600 px-3 absolute text-center top-0 w-[94%] "
      >
        <i className=" text-2xl ri-arrow-down-wide-line"></i>
      </h5>

      {/* <h3 className="font-bold text-xl pb-5 pt-4">Waiting for Driver</h3> */}

      <div className='flex item-cnter justify-between'>
        <img className='h-20' src="data:image/jpeg;base64," alt="" />
        <div className='pr-3 pt-4 text-right'>
          <h2 className='text-lg font-medium'>{props.ride?.user.fullname.firstname} {props.ride?.user.fullname.lastname} </h2>
          <h4 className='text-sm font-bold'>{props.ride?.captain?.vehicle?.plate}</h4>
          <p className='font-light text-gray-600'>Maruti Suzuki</p>

          <div className="flex items-center justify-end mt-2">
            <span className="text-gray-600 text-sm">OTP:</span>
            <span className="text-lg font-bold ml-2">{props.ride?.otp}</span>
          </div>
        </div>
      </div>

      <div className="flex flex-col items-center justify-between gap-3">
        
        <div className=" w-full p-3 mt-2">
          <div className=" flex items-center gap-3 pb-3">
            <i className=" text-lg ri-map-pin-2-fill"></i>
            <div className="border-b">
              <h4 className="font-bold text-xl">
                {props.ride?.pickup}
              </h4>
              <span className="text-gray-400 text-sm">
                Dighi road, Vishrantwadi, Pune, Maharashtra
              </span>
            </div>
          </div>
          <div className="flex items-center gap-3 pb-3">
            <i className=" text-lg ri-square-fill"></i>
            <div className="border-b">
              <h4 className="font-bold text-xl">{props.ride?.destination}
              </h4>
              <span className="text-gray-400 text-sm">
                Dighi road, Vishrantwadi, Pune, Maharashtra
              </span>
            </div>
          </div>
          <div className="flex items-center gap-3 pb-3">
            <i className=" text-lg ri-wallet-fill"></i>
            <div >
              <h3 className="font-bold text-xl">₹{props.ride?.fare}</h3>
              <span className="text-gray-400 text-sm">Cash Cash</span>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}

export default WaitingForDriver